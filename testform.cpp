    ...
    ui(new Ui::testForm)
{
    ...
    ui->spinBox_4->setValue(static_cast<int>(QDateTime::currentMSecsSinceEpoch()/1000));
...
void testForm::on_pushButton_clicked()
{
    emit ss(static_cast<uint>(ui->spinBox->value()),static_cast<uint>(ui->spinBox_2->value()),
            static_cast<uint>(ui->checkBox->isChecked()?ui->dateTimeEditS->dateTime().toMSecsSinceEpoch()/1000:ui->spinBox_3->value()),
            static_cast<uint>(ui->checkBox->isChecked()?ui->dateTimeEditE->dateTime().toMSecsSinceEpoch()/1000:ui->spinBox_4->value()),
